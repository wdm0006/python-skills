...
# Give a CI job exactly the credentials it needs
/plugin install permissioning-ci-workflows@dev-skills
...
| **permissioning-ci-workflows** | Give a CI job exactly the credentials it needs and know when it has none — explicit `permissions:` blocks instead of the repo-wide default token scope, the 403's `x-accepted-github-permissions` header naming the missing scope rather than guessing it, a PR comment needing `issues: write` *and* `pull-requests: write`, a job-level block that replaces the workflow floor rather than merging with it, fork pull requests running with a read-only token and no repository secrets, steps skipped on an empty secret and reported as passing, event-conditioned steps that skip on the `synchronize` re-run and turn the job green without fixing anything, why `pull_request_target` is not the workaround, and preferring a check decidable from repo contents over one that needs a token | — |
...
| **normalizing-text-for-measurement** | Strip, parse, and segment a document before something else measures it — a close token that emits no separator fusing `# Title` into the paragraph below it so word counts drop and readability worsens with heading density, the two metric classes failing on different residue — character-based ones (reading time, size limits, token estimates) inflated by link targets and table pipes, word-based ones (word count, readability) corrupted by missing block boundaries — a sibling function that never calls the normalizer at all, a bare parser constructor whose preset reclassifies a table as a paragraph, sections keyed by heading text that collapse on repeats and double-count into an ancestor, deliberate lossiness indistinguishable from a bug, and regression tests that assert markdown-equals-plain-equivalent rather than a float from an unpinned library | — |
...
- **permissioning-ci-workflows** — workflow steps that comment on a PR, push a commit, upload a report, or call another repository's API (declare `permissions:` explicitly, read the missing scope out of the 403 header, know which gates do nothing on a fork PR, never run head-ref content under `pull_request_target`, prefer checks that need no credential).
